...
- [ ] 每项行为修改以代表性失败用例开始，最小实现后通过对应测试；各完整阶段及交付前运行 npm run check。最终联网完整检查：MD2WECHAT_LIVE_LAYOUT_TEST=1 npm run check，23 个文件、140/140 项通过，包含真实五类模块；类型检查与构建通过。默认不开启真实模块测试时为 139 项通过、1 项跳过。
...
- [x] npm run package；检查 ZIP 包含三个插件文件与配套技能，不含 data.json、凭证、用户文章、生成候选或本地测试资料。本地必需内容已核验，README 图片资源与验收说明补齐后的最终文件清单另行复核，尚未发布。
...
最终联网完整自动检查：23 个文件、140/140 项通过，包含真实五类模块，类型检查和构建通过。默认检查为 139 项通过、1 项主动开启项跳过。本地安装包必需内容已核验，无 data.json 和测试资料；README 资源及验收说明补齐后的最终清单另行复核，不记录易变化的 ZIP 大小。没有发布软件版本。
